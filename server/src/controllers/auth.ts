...
class AuthController {
  ...
  async login(req: Request, res: Response) {
    try {
      ...
      if (!foundUser) {
        return res.status(400).json({ message: "Invalid credentital - User not found" });
      }
      ...
      if (!validPassword) {
        return res.status(400).json({ message: "Invalid credentital - Wrong password" });
      }
